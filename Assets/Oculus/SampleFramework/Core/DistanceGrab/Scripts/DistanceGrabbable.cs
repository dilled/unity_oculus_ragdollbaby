...
namespace OculusSampleFramework
{
    public class DistanceGrabbable : OVRGrabbable
    {
        ...
        private void Update()
        {
            if (isGrabbed && !firstGrab)
            {

                if (GetComponentInParent<BabyAngryController>() != null)
                {
                    GetComponentInParent<BabyAngryController>().Grabbed();
                    firstGrab = true;
                    //GetComponentInParent<Animator>().SetTrigger("grabbed");
                    GetComponentInParent<Animator>().enabled = false;
                    //GetComponentInParent<RagdollScript>().Grabbed();
                    //Debug.Log("GRAB!!!!");
                }               
            }
            if (!isGrabbed && firstGrab)
